# Teich — Ablation Protocol (v1.6)

**Status:** v1.6 — margins unchanged since v1.0; execution moved to Cloudflare Workers
...
system prompts are byte-identical; Mouth checkpoint corrected to
Llama-3.1-70B-Instruct (v1.6, latency-forced, still before any scored
conversation under the NIM instrument).** v1.0–v1.4 were all anchored BEFORE the first
...
- Mouth: **NVIDIA NIM `meta/llama-3.1-70b-instruct`** (v1.6 — plain instruct build, no
  thinking mode; v1.5's `qwen/qwen3-next-80b-a3b-instruct` proved congestion-prone for
  trial keys, hanging sockets mid-conversation — replaced after live latency probes,
  before any scored conversation under the NIM instrument), temp 0.7, max 160 tokens,
  seed = f(arm, script, turn) fixed a priori;
...
- Judge: **NVIDIA NIM `mistralai/mistral-small-4-119b-2603`** (v1.5 — same
  Mistral-small family as the v1.1 CF judge) — a **different model family from the Llama
...
| 2026-07-20 | v1.6 | Mouth checkpoint within the NIM backend: `qwen/qwen3-next-80b-a3b-instruct` → `meta/llama-3.1-70b-instruct`. Live probes showed the Qwen endpoint (and several other new-generation endpoints) hanging trial-key requests at the socket for minutes — the v1.5 pilot could not finish one conversation. Llama-3.1-70B answered a 12-call burst with zero hangs and passed the Mouth fidelity check (exact saddle quote, correct wing sign, lawful identity defense). Judge unchanged; families still distinct (Meta vs Mistral). No scored conversation had run under v1.5, so the scored set still starts from zero under one uniform instrument. Harness `_call` timeout 180 s → 90 s (fail fast + retry re-enters the queue). | Same forced-instrument-replacement class as v1.5; empirical availability governs model choice on free tiers. Anchored before the first scored conversation of the NIM era. |
