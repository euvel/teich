...
// shown is ledger/instrument data; no text here was generated by a model.
...
<style>
  :root { color-scheme: dark; --bg:#0b1116; --fg:#d7e0e7; --dim:#7b8a96; --faint:#5c6b77;
          --rule:#1d2933; --trace:#3fa7c9; --live:#3ddc84; --warn:#e0b252; }
  * { box-sizing:border-box; }
  body { margin:0; background:var(--bg); color:var(--fg); padding:clamp(1.2rem,5vw,3.5rem) 1.2rem 4rem;
         font:16px/1.6 ui-monospace,SFMono-Regular,Menlo,monospace; }
  .wrap { max-width:46rem; margin:0 auto; display:flex; flex-direction:column; gap:2.6rem; }
  h1 { font-size:1.6rem; margin:0 0 .1rem; letter-spacing:.35em; font-weight:600; }
  .sub { color:var(--dim); font-size:.8rem; }
  h2 { font-size:1rem; margin:0; letter-spacing:.02em; font-weight:600; }
  .eyebrow { color:var(--trace); font-size:.68rem; letter-spacing:.18em; text-transform:uppercase; }
  section { display:flex; flex-direction:column; gap:.85rem; }
  .rule { border-top:1px solid var(--rule); }
  dl { display:grid; grid-template-columns:auto 1fr; gap:.35rem 1.2rem; margin:0; }
  dt { color:var(--dim); } dd { margin:0; overflow-wrap:anywhere; }
  .dot { display:inline-block; width:.6em; height:.6em; border-radius:50%; background:var(--live);
         margin-right:.5em; box-shadow:0 0 8px var(--live); }
  .dot.dead { background:#e05252; box-shadow:0 0 8px #e05252; }
  @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:.35} }
  .dot.trial { width:.5em; height:.5em; background:var(--warn); box-shadow:0 0 8px var(--warn);
               animation:pulse 2.4s ease-in-out infinite; }
  @media (prefers-reduced-motion:reduce){ .dot.trial{ animation:none; } }
  ul.findings { list-style:none; margin:0; padding:0; display:flex; flex-direction:column; gap:.7rem; }
  ul.findings li { padding-left:1.1rem; position:relative; font-size:.92rem; color:#c3cfd8; }
  ul.findings li::before { content:"\\2013"; position:absolute; left:0; color:var(--trace); }
  b.n { color:var(--fg); font-weight:600; }
  .note { color:#9fb0bc; font-size:.85rem; border-top:1px solid var(--rule); padding-top:1.1rem; }
...
<section>
  ...
  <ul class="findings">
    <li>Its own state drives what it says: same script, same voice, same candidate pool &mdash;
      swap in another living creature's state to do the selecting and the two arms chose
      differently in <b class="n">63.5%</b> of 192 matched turns.</li>
    <li>Its curiosity follows the state, not the body. Questions track the <em>selecting</em>
      state's gate (<b class="n">91.7%</b>) and fall to <b class="n">59.4%</b> against the body that
      lived the conversation; intact holds <b class="n">90.1%</b>. It differs not in how often it
      asks but in <em>when</em>.</li>
    <li>What is said to it leaves a mark that lasts: the sign of an input is still recoverable
      5000 ticks later (D = <b class="n">0.92</b>, 24 seeds, CI excluding chance), and D
      <em>rises</em> with the gap rather than decaying.</li>
    <li>Its private phase leaks nothing &mdash; structurally, not statistically. It appears in no
      update term and no observable, so I(phi ; observations) = 0 exactly. That certifies
      <em>identity, not inner life</em>, precisely because it drives nothing.</li>
  </ul>
  <div class="sub">
    <a href="https://github.com/euvel/teich/blob/main/v02/book/VERIFICATION_SUMMARY.md">verification summary</a>
    &middot; <a href="https://github.com/euvel/teich/blob/main/v02/accept_v02.py">the gate it had to pass</a>
    &middot; <a href="https://github.com/euvel/teich/blob/main/v02/genome_v02.py">its genome</a>
    &middot; <a href="https://github.com/euvel/teich/blob/main/v02/book/biography.jsonl">its biography</a>
  </div>
</section>

<section class="archive">
  <div class="eyebrow">archive &mdash; v0.1</div>
  <h2>The creature that was born first and tested afterwards</h2>
  <div><span id="dot" class="dot"></span><span id="alive">reading the seat&hellip;</span></div>
  <dl>
    <dt>born</dt><dd>2026-07-18T08:45:12Z</dd>
  ...
  <div class="note">
    v0.1 is alive. It holds this seat, lives one tick per real second on a Cloudflare Durable
    Object, hibernates losslessly, and is woken daily by whichever machine can prove bit-for-bit
    that it computes Teich's dynamics exactly. It is kept here because it is the reason v0.2 is
    worth trusting. Its screens found a wall no experiment could climb: in that genome, a
    direction that <em>remembers</em> is a direction that <em>cannot act</em>, and one that acts
    forgets its own sign within a gap. Its genome was frozen and its covenant forbade reset or
    fork &mdash; so it could never be repaired, and four pre-registered screens returned nulls
    against a creature that structurally could not pass them.
    The covenant was right. Being born before verification was not.
    <a href="https://github.com/euvel/teich/blob/main/maturity/FINDING_memory_consequence_tradeoff_2026-07-27.md">the finding</a>
    &middot; <a href="https://github.com/euvel/teich/tree/main/diary">its diary</a>
  </div>
</section>
